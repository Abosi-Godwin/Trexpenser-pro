...
const SavingsInfo = () => {
  const { target_amount, amount_saved, start_date, end_date, method, portalRef } =
    useSavingsContext();
...
const SavingsForm = () => {
  ...
  const { method, percentage, funded_by, id, amount_saved, isActive } = useSavingsContext();
      ...
      {method === "manual" ? (
        ...
        >
          <Input
            inputType="number"
            placeholder={
              isActive === "active" ? "Add to your savings..." : getPlaceholderText(isActive)
            }
            label="amount"
        ...
        <>
          <p
            className="p-2 bg-light-sectionBackground rounded-md
                        dark:bg-dark-sectionBackground"
          >
            {percentage}% of each {funded_by} income will be added to this savings goal.
          </p>
        </>
      )}
